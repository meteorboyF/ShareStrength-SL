...
window.VoiceTracking = {
    start,
    stop,
    toggle: () => activeConfig.enabled ? stop() : start(),
    emergencyRecovery: forceRestart,
    testLeteredOverlays: () => {
        showNotification("Test overlays");
    },
    extendTimeout: extendLeteredTimeout,
    debugLeteredState: () => console.log(state),
    getPerformanceInfo: () => ({
        timersActive: state.timers.size,
        eventListenersActive: state.eventListeners.size
    }),
    getErrorInfo: () => ({
